...
class TSP:
    ...
    def optimize(self, filename: str):
        ...
        # These parameters can be auto-set based on problem size if desired
        n = D.shape[0]
        if 1 <= n <= 60:
            self.mu = 100 
            self.lamb = 100
        ...
        while yourConvergenceTestsHere:
            ...
            if self.stagnation_counter >= STAGNATION_LIMIT:
                ...
                k_dest = max(5, int(0.1 * self.mu)) # apply to top 10% or at least 5 individuals 
    ...
    def _compute_fitness_population(population: list[np.ndarray], D: np.ndarray) -> np.ndarray:
        ...
        return np.array([TSP._tour_length(t, D) for t in population], dtype=float)
